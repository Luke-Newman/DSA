...
/** AVL tree not created by me, sued as template and added the functionality I needed **/

#ifndef IDEASBANK_V1_AVL_TREE_H
#define IDEASBANK_V1_AVL_TREE_H
...
private:
    ...
    // added by Luke
    void _traverseAndRemoveId (int &id, vector<string> &keysToDelete, NODE<TYPE> *root);
    void _removeIdFromNode(TYPE &k, int id, vector<string> &keywordsToDelete);
...
// Added by Luke
    vector<string> traverseMarkForRemoval (int id);
    bool idInList(int id, vector<int>idList);
} ; // class AvlTree
...
template <class TYPE, class KTYPE> // TODO: change the way data is inserted, add function
...
//	Statements
    ...
    else if (newPtr->data.key > root->data.key) /// I changed from >= to >
        //  new data > root data
    {
    ...
    else if (newPtr->data.key == root->data.key)/// the key of data is equal, append idea ID to node, no balancing required
    {
        int id = newPtr->data.idList.back(); // will be the only ID in vector anyway
        if (!idInList(id, root->data.idList)) {
            root->data.idList.push_back(id);
        }
    } //  else new data key == root data key
    return root; // may need to be moved into last else statement
}	//  _insert
...
/* =========== Traverse and mark for removal =======
    Added by Luke; traverse and remove id from key,
    if there are no id's left in the id list, the key
    is marked for removal;
*/

// public function
template <class TYPE, class KTYPE>
vector<string> AvlTree<TYPE, KTYPE>
::traverseMarkForRemoval (int id)
{
    vector <string> keysToDelete;
    _traverseAndRemoveId(id, keysToDelete, tree);
    return keysToDelete;
};

// private recursive traversal function
template <class TYPE, class KTYPE>
void AvlTree<TYPE, KTYPE>::_traverseAndRemoveId (int &id, vector<string> &keysToDelete, NODE<TYPE> *root)
{
    if (root)
    {
        _traverseAndRemoveId  (id, keysToDelete, root->left);
        _removeIdFromNode    (root->data, id, keysToDelete);
        _traverseAndRemoveId  (id, keysToDelete, root->right);
    }
    return;
};

// private member to remove id from node, if no ID's left in node delete
template <class TYPE, class KTYPE>
void AvlTree<TYPE, KTYPE>::_removeIdFromNode(TYPE &k, int id, vector<string> &keywordsToDelete) {

    for (int i = 0; i < k.idList.size(); i++) {
        if (k.idList[i] == id) {
            swap(k.idList[i], k.idList.back());
            k.idList.pop_back();
            break;
        }
    }
    ...
    if (k.idList.empty()) {
        keywordsToDelete.push_back(k.key);
    }
                                       ...
                                       int         level)
{
    ...
    if (root)
    {
        ...
        for (i = 0; i <= level; i++ )
            cout << "...."; // TODO added occurrence here
        cout << setw(3) << root->data.key << " ";
        for (int id : root->data.idList) { cout << id << " ";}
...
template <class TYPE, class KTYPE>
bool AvlTree<TYPE, KTYPE>::idInList(int id, vector<int> idList) {
